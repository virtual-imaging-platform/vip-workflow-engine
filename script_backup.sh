...
# Creating the path to the configuration JSON file
configurationJson="config/$filename-configuration.json"

# Read JSON keys dynamically and assign values to variables

read_variables() {
    local variable="$1"  # Assign the input parameter to a local variable

    # Read key-value pairs dynamically and assign values to variables
    while IFS="=" read -r key value; do
        ...
        if [[ -n $key ]]; then
            ...
            declare -g "$key=$value"  # Declare variables in the global scope
        fi
    done < <(jq -r ".$variable | to_entries | .[] | \"\(.key)=\(.value)\"" "$configurationJson")
}

# Call the function for each configuration section
read_variables "gaswConfiguration"
read_variables "gaswConstants"
read_variables "jobConfiguration"
...
upload() {
    ...
    echo "upload, URI is ${URI}"
    echo "upload, FILENAME is ${FILENAME}"
    echo "upload, ID is ${ID}"
    echo "upload, NREP is ${NREP}"
    echo "upload, TEST is ${TEST}"
...
do
    echo $output_to_be_uploaded + $uploads + ${numberOfReplicas} + TTTTTTTTTTTTTTTTTTTTT
